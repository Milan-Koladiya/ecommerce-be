...
const sendVerificationEmail = async (user:any, origin:any) => {
    const now = Date.now();
...
const sendEmail = async (to:any, firstName:string, lastname:string, verificationUrl:any) => {
    const mailOptions = {
...
const sendResetPasswordEmail = async (email:any, firstName:string, lastName:string, resetUrl:any) => {
    const mailOptions = {
        from: process.env.EMAIL_USER,
        to: email,
